THREE.GLTFLoader = function() {
	...
	e.prototype = {
		...
		parse: function(e, o, l, p) {
			...
			if ("string" == typeof e)
				...
				u = THREE.LoaderUtils.decodeText(new Uint8Array(e));
			var d = JSON.parse(u);
			void 0 === d.asset || d.asset.version[0] < 2 ? p && p(new Error("THREE.GLTFLoader: Unsupported asset. glTF versions >=2.0 are supported. Use LegacyGLTFLoader instead.")) : (d.extensionsUsed && (d.extensionsUsed.indexOf(t.KHR_LIGHTS) >= 0 && (c[t.KHR_LIGHTS] = new function(e) {
				...
				return {
					...
					cloneMaterial: function(e) {
						...
						for (var a = this.specularGlossinessParams, r = 0, s = a.length; r < s; r++)
							t[a[r]] = e[a[r]];
						return t
					},
						...
						if (!0 === s.isGLTFSpecularGlossinessMaterial) {
							...
							if (p.opacity.value = s.opacity, p.diffuse.value.copy(s.color), p.emissive.value.copy(s.emissive).multiplyScalar(s.emissiveIntensity), p.map.value = s.map, p.specularMap.value = s.specularMap, p.alphaMap.value = s.alphaMap, p.lightMap.value = s.lightMap, p.lightMapIntensity.value = s.lightMapIntensity, p.aoMap.value = s.aoMap, p.aoMapIntensity.value = s.aoMapIntensity, s.map ? n = s.map : s.specularMap ? n = s.specularMap : s.displacementMap ? n = s.displacementMap : s.normalMap ? n = s.normalMap : s.bumpMap ? n = s.bumpMap : s.glossinessMap ? n = s.glossinessMap : s.alphaMap ? n = s.alphaMap : s.emissiveMap && (n = s.emissiveMap), void 0 !== n)
								if (n.isWebGLRenderTarget && (n = n.texture), void 0 !== n.matrix) {
									...
									o = n.offset, l = n.repeat, p.offsetRepeat.value.set(o.x, o.y, l.x, l.y);
	...
	function T(e, t, a, r) {
		...
		for (var l = 0, p = n.length; l < p; l++) {
			...
			if (void 0 !== d.POSITION) {
				u = M(r[d.POSITION]);
				for (var E = s.attributes.position, m = 0, f = u.count; m < f; m++)
					u.setXYZ(m, u.getX(m) + E.getX(m), u.getY(m) + E.getY(m), u.getZ(m) + E.getZ(m))
			} else
				s.attributes.position && (u = M(s.attributes.position));
			if (void 0 !== u && (u.name = h, o.position.push(u)), void 0 !== d.NORMAL) {
				i.morphNormals = !0, c = M(r[d.NORMAL]);
				var v = s.attributes.normal;
				for (m = 0, f = c.count; m < f; m++)
					c.setXYZ(m, c.getX(m) + v.getX(m), c.getY(m) + v.getY(m), c.getZ(m) + v.getZ(m))
			} else
				void 0 !== s.attributes.normal && (c = M(s.attributes.normal));
			void 0 !== c && (c.name = h, o.normal.push(c))
		}
		if (e.updateMorphTargets(), void 0 !== t.weights)
			for (l = 0, p = t.weights.length; l < p; l++)
				e.morphTargetInfluences[l] = t.weights[l];
		if (t.extras && Array.isArray(t.extras.targetNames))
			for (l = 0, p = t.extras.targetNames.length; l < p; l++)
				e.morphTargetDictionary[t.extras.targetNames[l]] = l
	}
	function R(e, t) {
	...
	function M(e) {
		if (e.isInterleavedBufferAttribute) {
			for (var t = e.count, a = e.itemSize, r = e.array.slice(0, t * a), s = 0; s < t; ++s)
				r[s] = e.getX(s), a >= 2 && (r[s + 1] = e.getY(s)), a >= 3 && (r[s + 2] = e.getZ(s)), a >= 4 && (r[s + 3] = e.getW(s));
	...
	return S.prototype.parse = function(e, t) {
		...
		for (var e = this.json.nodes || [], t = this.json.skins || [], a = this.json.meshes || [], r = {}, s = {}, i = 0, n = t.length; i < n; i++)
			for (var o = t[i].joints, l = 0, p = o.length; l < p; l++)
				e[o[l]].isBone = !0;
		...
		return void 0 !== r.bufferView ? s.push(this.getDependency("bufferView", r.bufferView)) : s.push(null), void 0 !== r.sparse && (s.push(this.getDependency("bufferView", r.sparse.indices.bufferView)), s.push(this.getDependency("bufferView", r.sparse.values.bufferView))), Promise.all(s).then(function(e) {
			...
			if (E && E !== c) {
				...
				s = null === n ? new p(r.count * o) : new p(n, d, r.count * o), i = new THREE.BufferAttribute(s, o, m);
			if (void 0 !== r.sparse) {
				var T = h.SCALAR,
				...
				for (var y = 0, L = S.length; y < L; y++) {
					var x = S[y];
					if (i.setX(x, H[y * o]), o >= 2 && i.setY(x, H[y * o + 1]), o >= 3 && i.setZ(x, H[y * o + 2]), o >= 4 && i.setW(x, H[y * o + 3]), o >= 5)
		...
		return this.getDependencies("accessor").then(function(i) {
			for (var n = [], o = [], l = 0, p = e.length; l < p; l++) {
				...
				if (d)
					...
					H(u = new THREE.BufferGeometry, c, i), s.push({
						primitive: c,
		...
		return this.getMultiDependencies(["accessor", "material"]).then(function(i) {
			...
			return a.loadGeometries(o).then(function(l) {
				for (var p = 0, u = o.length; p < u; p++) {
					...
					var E,
						m = void 0 !== d.attributes.color,
						f = void 0 === d.attributes.normal,
						v = !0 === s.isSkinnedMesh,
						R = void 0 !== c.targets;
					if ((m || f || v || R) && (h = h.isGLTFSpecularGlossinessMaterial ? r[t.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS].cloneMaterial(h) : h.clone()), m && (h.vertexColors = THREE.VertexColors, h.needsUpdate = !0), f && (h.flatShading = !0), 4 === c.mode || 5 === c.mode || 6 === c.mode || void 0 === c.mode)
						v ? (E = new THREE.SkinnedMesh(d, h), h.skinning = !0) : E = new THREE.Mesh(d, h), 5 === c.mode ? E.drawMode = THREE.TriangleStripDrawMode : 6 === c.mode && (E.drawMode = THREE.TriangleFanDrawMode);
					...
					if (E.name = s.name || "mesh_" + e, R && T(E, s, c, i.accessors), void 0 !== s.extras && (E.userData = s.extras), void 0 !== c.extras && (E.geometry.userData = c.extras), !0 === h.isGLTFSpecularGlossinessMaterial && (E.onBeforeRender = r[t.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS].refreshUniforms), !(o.length > 1))
						return E;
					E.name += "_" + p, n.add(E)
				}
				return n
		...
		if (r) {
			if ("perspective" === a.type) {
				...
				"orthographic" === a.type && (t = new THREE.OrthographicCamera(r.xmag / -2, r.xmag / 2, r.ymag / 2, r.ymag / -2, r.znear, r.zfar));
		...
		return this.getMultiDependencies(["mesh", "skin", "camera"]).then(function(e) {
			...
			if (!0 === i.isBone)
				...
				if (n = o.clone(), !0 === o.isGroup)
					...
					o.material && !0 === o.material.isGLTFSpecularGlossinessMaterial && (n.onBeforeRender = o.onBeforeRender);
				r[i.mesh] > 1 && (n.name += "_instance_" + s[i.mesh]++)
			} else
				n = void 0 !== i.camera ? e.cameras[i.camera] : i.extensions && i.extensions[t.KHR_LIGHTS] && void 0 !== i.extensions[t.KHR_LIGHTS].light ? a[t.KHR_LIGHTS].lights[i.extensions[t.KHR_LIGHTS].light] : new THREE.Object3D;
			if (void 0 !== i.name && (n.name = THREE.PropertyBinding.sanitizeNodeName(i.name)), i.extras && (n.userData = i.extras), void 0 !== i.matrix) {
				...
				void 0 !== i.translation && n.position.fromArray(i.translation), void 0 !== i.rotation && n.quaternion.fromArray(i.rotation), void 0 !== i.scale && n.scale.fromArray(i.scale);
			return n
		})
				...
				for (var l = !0 === n.isGroup ? n.children : [n], p = 0, u = l.length; p < u; p++) {
					for (var c = l[p], d = i[o.skin], h = [], E = [], m = 0, f = d.joints.length; m < f; m++) {
						...
						if (T) {
							...
							console.warn('THREE.GLTFLoader: Joint "%s" could not be found.', v)
					}
		...
		return function(a) {
			...
			return this.getMultiDependencies(["node", "skin"]).then(function(a) {
				...
				for (var o = i.nodes || [], l = 0, p = o.length; l < p; l++)
					e(o[l], n, r, a.nodes, a.skins);
